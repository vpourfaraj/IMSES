...
kumu.data.e<-read_excel("kumu_test_jct01.xlsx", sheet=1)#reads element table from kumu output
kumu.data.c<-read_excel("kumu_test_jct01.xlsx", sheet=2)#reads connections table from kumu output
kumu.data.c<-kumu.data.c[,c(1,2,8)] #save columns "From", "To" and "Strength"
kumu.graph<- graph_from_data_frame(kumu.data.c,directed = TRUE, vertices = NULL ) #this line added to convert data to a graph object for next line
...
nodes<-kumu.data.e %>% 
  select(Label,Type) %>% 
   ...
   add_column (id=1:57, font.size=40)%>%
    arrange(id, label, group, font.size)
...
IMSES_net<-visNetwork(nodes, edges, height = "600px", width="100%") %>%
  visIgraphLayout() %>%
  visEdges(arrows="to") %>%
  visNodes(size = 20) %>%
